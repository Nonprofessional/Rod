...
profile (`-mode stream` holds the connection, `-mode poll` contacts on the
...
Add `-mode poll` for the low-and-slow contact cadence (sleep/jitter between
contacts instead of a persistent connection); the default `-mode stream` is the
interactive shape.
