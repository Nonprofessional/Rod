...
// runtime discovery buys nothing. The reference registrations live in
// HandlerSelection -- adding a verb is a handler plus one registration there,
// never an edit to the beacon loop. Out-of-tree handlers compile in through
// the extension kit's generated Extensions/ExtensionRegistrations (the build
// unit overlays the configured extension directory onto the staging tree and
// generates the registrations feeding the additional seam,
// extending/tradecraft.md); the reference registry carries no Sec 13 boundary
// verb.
...
/// </summary>
/// <remarks>
/// Lives here, beside the channel registration shape that consumes it, and
/// not in any one channel handler's source: the tunnel verbs and the
/// interactive shell both run over it, so it must survive the bake-time
/// handler trim no matter which channel verbs a reduced class keeps.
/// </remarks>
...
internal sealed class HandlerRegistry
{
    ...
    /// <summary>
    /// Builds the reference registry from the compiled handler selection: the
    /// registrations <see cref="HandlerSelection"/> carries -- the full
