...
/// core set plus the recon set; recon is a long-haul activity. These sets are
...
public static class ImplantClassCapabilities
{
    ...
    /// full core set plus the recon set; every other class carries the subset its
    /// operational purpose justifies (architecture.md Sec 5.2). Stored read-only and
    /// case-normalized so <see cref="Allows"/> can match case-insensitively
    ...
    private static readonly IReadOnlyDictionary<ImplantClass, IReadOnlyList<string>> ByClass =
        new Dictionary<ImplantClass, IReadOnlyList<string>>
        {
            ...
            // recon set (architecture.md Sec 10.1). Recon is a long-haul
            // activity -- it justifies a stage-2 footprint and no other class.
