...
// in the shell families a target is likely to have, mirroring the console's
// Upgrade launcher rendering. Pure rendering of (host, port) into commands
// -- no credential is minted, so the client renders from the roster it
// already holds.
function catchLaunchers(
  ...
  return [
    ...
    // refuses it, so the fifo shape rides beside it. Showing both costs
    // nothing while a missing one costs a round trip.
