...
/// <summary>
/// Roadmap M3.2 acceptance: the reference Go implant checks in and tasks
/// end-to-end. A real teamserver (Kestrel mTLS beacon endpoint + plain-HTTP
/// operator/enroll API) is stood up, the reference Go implant is launched against
/// it as a real subprocess (go run), and the test drives the full slice the AC
/// names: the implant enrolls over HTTP, beacons over mTLS, completes the
/// handshake, runs a dispatched shell.exec task, and the operator sees the
/// captured output plus the TaskCompleted audit event. This is the same round-trip
/// HandshakePresenceTests/TaskRoundTripTests prove in-process, now driven by the
/// actual Go binary.
/// </summary>
public class GoImplantTests
{
    /// <summary>
    ...
    /// </summary>
    [GoFact]
    public async Task GoImplant_DerivesChildOnLateralMove_EndToEnd()
    {
        await using var env = await TestEnv.StartAsync();

        // One engagement, two single-use tokens: the parent redeems one at its own
        // enroll; the child redeems the second when the parent derives it. The
        // engagement id is resolved from the parent's enrollment (the token carries
        // it), so mint both tokens against the same engagement up front.
        var (engagementId, parentToken, childToken) = await env.MintEngagementWithTwoTokensAsync();

        var implantSource = LocateImplantSource();
        var implantBinary = BuildImplantBinary(implantSource);
        var implantProc = StartImplant(implantBinary, env, parentToken,
        ...
        using (implantProc)
        {
            ...
            finally
            {
                ...
                try { if (File.Exists(implantBinary)) File.Delete(implantBinary); } catch { }
            }
        }
    }

    [GoFact]
    public async Task GoImplant_EnrollsBeaconsAndTasks_EndToEnd()
    {
        await using var env = await TestEnv.StartAsync();
        ...
        // Launch the reference implant as a real subprocess pointed at the test
        // server: enroll over HTTP, beacon over mTLS. A short sleep keeps the
        // check-in prompt so the round-trip resolves quickly.
        //
        // The implant is built once for the whole test into a temp binary and run
        // as a real subprocess. We build-and-run rather than `go run` because
        // `go run` resolves its module graph against GOPROXY on every invocation,
        // which hangs silently when the host has no outbound network -- the build
        // is offline-safe (modules verified), the run is then a plain exec.
        var implantSource = LocateImplantSource();
        var implantBinary = BuildImplantBinary(implantSource);
        var implantProc = StartImplant(implantBinary, env, secret,
            sleep: TimeSpan.FromSeconds(1), jitter: TimeSpan.Zero);
        ...
        // implant process is disposable.
    }

    // Walks up from the test assembly to find the repo root (the directory holding
    // both 'src' and 'implant'), then returns the implant source tree. The Go
    ...
    private static string BuildImplantBinary(string implantSource)
    {
        ...
        // .NET ProcessStartInfo composes the argument list verbatim; go build's
        // -o takes the output path as the next argument.
        ...
        psi.ArgumentList.Add("-mod=readonly");
        ...
        build.WaitForExit();
        if (build.ExitCode != 0)
            throw new InvalidOperationException($"go build failed (exit {build.ExitCode}):\n{buildErr}");
        return binary;
    }

    // Starts the reference implant binary as a real subprocess. The binary
    // enrolls over HTTP and beacons over mTLS against the test server.
    ...
    private static Process StartImplant(string implantBinary, TestEnv env, string token, TimeSpan sleep, TimeSpan jitter)
    {
        var psi = new ProcessStartInfo
        {
    ...
    // Formats a TimeSpan the way Go's time.ParseDuration reads it: an integer
    // count of seconds with an "s" unit (e.g. "1s", "0s"). TimeSpan.ToString
    // emits "00:00:01", which ParseDuration rejects. The test only ever passes
        ...
        TestEnv env, TimeSpan deadline, StringBuilder stderr)
    {
        ...
        // Iterate the operator engagements to find the one with an online implant.
        // The test minted the token for a fresh engagement; that engagement is the
        // only one with an implant, so it surfaces here.
        ...
        throw new TimeoutException(
            "The Go implant did not appear online within the deadline. Implant stderr:\\n" + stderr);
    }
    ...
    private sealed class TestEnv : IAsyncDisposable
    {
        ...
        // by GoImplant_DerivesChildOnLateralMove_EndToEnd.
