...
public class DotNetBuildUnitTests
{
    ...
    [Theory]
    [InlineData(ImplantClass.Stage2, "shell.exec,shell.interact,file.push,file.pull,fs.list,proc.kill,beacon.sleep,tunnel.forward,tunnel.socks,recon.portscan,recon.hostenum,recon.service,recon.ps,lateral.move,lateral.token,lateral.exec_remote,persist.install,persist.remove,persist.list,collect.cred,collect.keylog,collect.screenshot,collect.minidump,inject.shellcode,exfil.push,exfil.stage,evasion.avoid,evasion.unload,exploit.invoke,exploit.module")]
