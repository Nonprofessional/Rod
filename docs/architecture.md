...
carries the full core set plus the recon set, the lateral set, and the persist
set (recon, lateral movement, and persistence are long-haul activities that
justify a stage-2 footprint); a stager only `file.pull`s the stage-2 it loads;
a web-shell and an ephemeral run `shell.exec` and `probe.read` over their
short-lived channels; a pivot carries `tunnel.open` and `probe.read` and no
shell. No class but Stage-2 carries a recon, lateral, or persist verb. The set
