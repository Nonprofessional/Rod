...
carries the full core set plus the recon set (recon is a long-haul activity that
justifies a stage-2 footprint); a stager only `file.pull`s the stage-2 it loads;
...
shell. No class but Stage-2 carries a recon verb. The set is the server's
...
pipeline bakes it into each artifact so a generated payload is self-describing.
