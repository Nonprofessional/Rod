...
> **Note:** this is a point-in-time snapshot. ADR 0009 later removed the
> in-tree Go implant and Go build unit; findings below that reference `implant/`
> or `GoBuildUnit` describe the baseline as of this date, not the current tree.
> The ADR files were folded into architecture.md on 2026-08-13 (see the former
> ADR index in architecture.md Sec 4.3), the redirector landed the same day, and
> the reference implant shipped its standard-category handlers on 2026-08-11 --
> findings 5, 16, and the Go references are superseded accordingly.
