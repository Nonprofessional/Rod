...
| **Collect** | The `collect.cred` and `collect.keylog` verbs (category `Collect`); credential and input collection within an authorized engagement, gated to Stage-2 at task issuance (Sec 5.2). The reference implant implements credential-store enumeration (`collect.cred`) without dumping secret material; `collect.keylog` stays contract-only (Sec 13). File transfer is a core verb (`file.push`/`file.pull`). |
