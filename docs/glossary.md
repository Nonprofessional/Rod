...
| **Collect** | The `collect.file`, `collect.cred`, and `collect.keylog` verbs (category `Collect`); file, credential, and input collection within an authorized engagement, gated to Stage-2 at task issuance (Sec 5.2). The reference implant implements `collect.file` and credential-store enumeration (`collect.cred`) without dumping secret material; `collect.keylog` stays contract-only (Sec 13). The descriptors and dispatch live in the tradecraft layer. |
